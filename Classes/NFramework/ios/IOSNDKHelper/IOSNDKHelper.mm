...
void IOSNDKHelperImpl::recieveBytes(void *data, int length){
    
    SenderParket* obj = [[SenderParket alloc] init];
    //NSData *_data = [[NSData alloc] initWithData:[data dataUsingEncoding:NSUTF8StringEncoding]];

    [obj.bytes appendBytes:data length:length];
    obj.length = length;
//
//    //send message
...
+(void)sendMessage:(int)length withData:(void *)prms
{
    // Send data to MobileClient
   // NetworkController::sharedNetworkController()->onReceivedByte((unsigned char*)prms,length);
}
